...
export async function AppdaemonCheck(context: Context, owner: string, repo: string) {
    ...
    const { data: CheckRun } = await context.github.checks.create(
        context.issue({
            head_sha: PRSHA,
            status: "in_progress",
            name: "AppDaemon repository checks",
            output: Summary,
    ...
    try {
        ...
        Summary.summary += "(https://hacs.xyz/docs/publish/appdaemon#repository-structure)";
        conclusion = "failure"
    }
