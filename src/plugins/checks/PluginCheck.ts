...
export async function PluginCheck(context: Context, owner: string, repo: string) {
    ...
    try {
        ...
        Summary.summary += "(https://hacs.xyz/docs/publish/plugin#repository-structure)";
        conclusion = "failure"
    }
    ...
    try {
        ...
        var EncodedReadme = await context.github.repos.getContents(
            {owner: owner, repo: repo, path: readme.path});
...
async function CheckDist(owner: string, repo: string, context: Context) {
    ...
    try {
        var DistContents = await context.github.repos.getContents(
            {owner: owner, repo: repo, path: "dist"});
