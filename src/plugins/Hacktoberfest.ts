...
async function CreateOrUpdateHacktoberfestLabel(context: Context) {
  ...
  const CurrentLabels = await context.github.issues.listLabelsForRepo(context.issue())

  CurrentLabels.data.forEach(element => {
    if (element.name.toLocaleLowerCase() === "hacktoberfest" ) LabelExists = true;
  });

  if (LabelExists) {
    await context.github.issues.updateLabel(
      context.issue({current_name: "hacktoberfest", name: "Hacktoberfest", color: "ff5500"}))
  } else {
