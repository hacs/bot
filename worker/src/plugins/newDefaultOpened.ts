...
export default async (app: App, payload: PullPayload): Promise<void> => {
  ...
  await app.octokit.rest.issues.addLabels(
    {...extractOwnerRepo(payload), issue_number: payload.pull_request.number, labels: ["New default repository"]}
  );

  await app.octokit.rest.issues.update(
    {...extractOwnerRepo(payload), issue_number: payload.pull_request.number, title: `Adds new ${repoCategory} [${owner}/${repo}]`}
  );

  await app.octokit.rest.issues.createComment(
    {...extractOwnerRepo(payload), issue_number: payload.pull_request.number, body: `Running checks on [${owner}/${repo}](https://github.com/${owner}/${repo})`}
  );
...
async function getFileDiff(app: App, payload: PullPayload, file: string) {
  const { data: currentContentData } = await app.octokit.rest.repos.getContent(
    { ...extractOwnerRepo(payload), pull_number: payload.pull_request.number, path: file }
  ...
  const { data: changedContentData } = await app.octokit.rest.repos.getContent(
    {...extractOwnerRepo(payload), issue_number: payload.pull_request.number, path: file, ref: payload.pull_request.head.sha}
